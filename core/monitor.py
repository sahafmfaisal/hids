...
HIDS_FILES = {"model.pkl","scaler.pkl","dataset.csv","monitor.py","train.py","app.py","hids.db"}
...
def proc_poller_thread():
    ...
    own_pid, own_ppid = str(os.getpid()), str(os.getppid())
    
    while True:
        pids = [p for p in os.listdir("/proc") if p.isdigit() and p not in (own_pid, own_ppid)]
        opens=0; reads=0; writes=0; execs=0; priv=0
        new_state={}; new_cmds=set()

        for pid in pids:
            try: opens += len(os.listdir(f"/proc/{pid}/fd"))
            except: pass
        ...
        proc_prev_state, proc_prev_cmds = new_state, new_cmds
...
def audit_tailer_thread():
    ...
    while True:
        ...
        if evt_id not in audit_cache:
            audit_cache[evt_id] = {}
            if len(audit_cache) > 500: audit_cache.pop(next(iter(audit_cache)))
...
def get_avg():
    ...
    if not scans: return {}
    return {k: sum(s.get(k,0) for s in scans)/len(scans) for k in scans[0].keys()}
    ...
    while True:
        ...
        if is_calib:
            ...
            if pred == 1 and in_dedup:
                ...
                evt = {**last_evt, "type":"suppressed", "timestamp":now.isoformat(), "reason":f"Threat persisting — {rem}s in dedup window"}
                try_db(evt)
                status = "suppressed"
                ...
                last_evt = {
                    "type":"alert","timestamp":now.isoformat(), "threat":threat,"severity":sev,
                    "confidence":round(conf,3), "mitre_id":mid,"mitre_name":mname,
